...
DEFAULT=dict(ocean='/project/engineering/cadence21/ic/tools/bin/ocean',
    ...
    with log.open('w') as output:
        try:
            with open(stdin) if stdin else open(os.devnull) as inp:
                return subprocess.run(argv,cwd=cwd,stdin=inp,stdout=output,stderr=subprocess.STDOUT,check=False).returncode
        except OSError as e:
            output.write(str(e)+'\n'); return 127
...
def run_one(job,cfg):
    ...
    try:
        ...
        candidates=list((out/'native').rglob('netlist'))
        # Locate only inside this attempt, never reuse ~/simulation's older netlist.
        candidates=[p for p in candidates if p.is_file()]
        if len(candidates)!=1: raise OSError('Expected one native netlist in isolated projectDir, found '+str(len(candidates)))
    ...
    write(out/'status.json',state)
    print(job['id'],state['status'],state.get('error',''),str(out),flush=True)
    return state
