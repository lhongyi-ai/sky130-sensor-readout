#!/usr/bin/env python3
"""Install passive review on an intact 1.0.4p1 runtime (Python 3.6+)."""
import hashlib
import json
import os
...
BASE_MANIFEST='c982275d2f5046439e77e4e5236d614d083e865fbb86414059a38d06f652aa66'
...
def install(root):
    ...
    if sha(manifest_path)==patch['payload']['package_manifest.json']:
        ...
        print('P1_RUNTIME_PATCH_ALREADY_APPLIED: 1.0.4p2')
        return
    if sha(manifest_path)!=BASE_MANIFEST:
        raise ValueError('This fix requires basic_design_v1_0_4 with runtime patch 1.0.4p1')
    verify_package(root,current)
    for name in ['passive_models.py','passive_review.py','passive_criteria.md']:
        if (root/name).exists() or (root/name).is_symlink():
            raise ValueError('Unmanaged file exists; preserve for review: '+name)
    backup=root/'patch_backups'/'v1_0_4p2'
    if backup.exists():
        raise ValueError('Previous patch backup exists; preserve it and report this message')
    backup.mkdir(parents=True)
    for name in ['run.py','analyze.py','runtime_patch.json','package_manifest.json']:
    ...
    try:
        ...
        raise
    print('P1_RUNTIME_PATCH_APPLIED: 1.0.4p2')
