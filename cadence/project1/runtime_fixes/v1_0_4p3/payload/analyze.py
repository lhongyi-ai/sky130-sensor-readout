...
def op(out,job):
    ...
    if len(devices)!=13 or {d['device'] for d in devices}!=expected:
        raise ValueError('Incomplete device OP: expected 13 rows, found '+str(len(devices))+'; missing '+str(sorted(expected-{d['device'] for d in devices})))
    if any(not math.isfinite(v) for d in devices for k,v in d.items() if k!='device'):
        raise ValueError('Nonfinite device OP value')
...
def analyze(out,job):
    ...
    if job['id'] in ['res_dc','mim_ac','rc_step']:
        from passive_models import analyze_passive
        return analyze_passive(out,job)
