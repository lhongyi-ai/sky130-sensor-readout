...
def read_plan(campaign):
    ...
    frozen_runner = plan.get("sources", {}).get("static_campaign.py")
    if frozen_runner and sha(__file__) != frozen_runner["sha256"]:
        raise ValueError("runner changed; invoke this campaign's frozen/static_campaign.py to preserve analysis semantics")
    ...
    try:
        ...
        if binary.exists():
            ...
            write_json(build_manifest, {"sha256": sha(binary), "plan_sha256": plan["plan_sha256"],
                                       "build_wall_seconds": time.monotonic() - build_began})
        performed = 0
...
def static_metrics(xs, codes):
    """Conservative grid brackets, endpoint INL and endpoint-LSB DNL intervals.

    Endpoint INL uses T1 and T4095; not affine calibration of ADC codes.
    DNL excludes unbounded saturating end codes, consistent with the project's
    endpoint_linearity analyser. Their accessibility is separately required.
    Nominal-LSB widths clipped to +/-0.4 V are diagnostics, not DNL signoff.
    ...
    lower = np.r_[-.4, lo, .4]
    upper = np.r_[-.4, hi, .4]
    nominal_dnl_lo = (lower[1:] - upper[:-1]) / LSB - 1
    nominal_dnl_hi = (upper[1:] - lower[:-1]) / LSB - 1
    fitted_lsb_lo, fitted_lsb_hi = (lo[-1] - hi[0]) / 4094, (hi[-1] - lo[0]) / 4094
    if fitted_lsb_lo <= 0:
        return {"status": "FAIL_NONPOSITIVE_ENDPOINT_SPAN"}
    width_lo, width_hi = lo[1:] - hi[:-1], hi[1:] - lo[:-1]
    dnl_corners = np.asarray([width / fit - 1 for width in (width_lo, width_hi) for fit in (fitted_lsb_lo, fitted_lsb_hi)])
    dnl_lo, dnl_hi = dnl_corners.min(axis=0), dnl_corners.max(axis=0)
    ...
    return {"status": "GRID_STATIC_LIMITS_PASS" if passed else "STATIC_LIMIT_FAIL" if definite_fail else "GRID_RESOLUTION_INCONCLUSIVE",
            ...
            "dnl_scope": "interior codes 1..4094, endpoint fitted LSB; codes 0 and 4095 separately observed",
            "nominal_lsb_clipped_width_diagnostic": [float(nominal_dnl_lo.min()), float(nominal_dnl_hi.max())],
...
def collect(campaign):
    ...
    for start in range(0, plan["required_points"], plan["batch_size"]):
        ...
        if report.get("plan_sha256") != plan["plan_sha256"]:
            ...
            continue
        if report.get("runtime_sha256") != sha(campaign / "runtime.json") or report.get("binary_sha256") != sha(campaign / "build/cosim_controller.so"):
            errors.append(f"batch {start}: runtime or compiled RTL identity mismatch")
            continue
