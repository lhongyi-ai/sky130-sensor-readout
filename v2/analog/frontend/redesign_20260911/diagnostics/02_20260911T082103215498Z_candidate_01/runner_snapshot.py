...
"""
    ...
    devices = {
        ...
        "xdut.xota.xbias.xdbp": "pfet_01v8",
    }
    for gain in GAINS:
        p = f"xg{gain}"
        nodes = [
            ...
            f"v({p}.xdut.xota.bp)", f"i(v.{p}.vddc)", f"i(v.{p}.vcmc)",
        ]
        op.extend(nodes)
        dc.extend(nodes)
...
def main() -> None:
    ...
    if not candidate.is_file() or not ADC_SOURCE.is_file():
        parser.error("candidate or ADC primitive source is missing")
