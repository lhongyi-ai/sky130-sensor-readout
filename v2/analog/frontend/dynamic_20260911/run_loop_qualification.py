...
def crossing_report(frequency: np.ndarray, z: np.ndarray) -> dict:
    ...
    up = [item for item in crossings if item["direction"] == "up"]
    low_phase_wrapped = float((phase[0] + 180) % 360 - 180)
    low_frequency_negative_feedback_sign = abs(low_phase_wrapped) <= 45
    return {
        "low_frequency_gain_db": float(magnitude_db[0]),
        "low_frequency_phase_deg": float(phase[0]),
        "low_frequency_phase_wrapped_deg": low_phase_wrapped,
        "low_frequency_negative_feedback_sign": low_frequency_negative_feedback_sign,
        ...
        "phase_margin_gate_pass": bool(
            low_frequency_negative_feedback_sign
            and down
            and not up
