#!/usr/bin/env python3
"""Bounded serial TT dynamic-closure diagnostic for the front end.

One invocation evaluates one frozen source snapshot at all three gains.  It is
an experiment, not signoff: no more than 8 timestamped diagnostics can be
...
def build_deck(candidate_name: str) -> tuple[str, list[str], list[str], list[str]]:
    case = """
.subckt rd_case params: SUP=1.8 G=4 S0=1.8 S1=0 VREF=0.9
VDDC VDD 0 {SUP}
VCMC VCM 0 {VREF}
...
* 1.5 kohm maps to L=0.543 um in rd_hr, above the 0.5 um PCell minimum.
* It reduces the real 4096-unit acquisition time constant versus 1.8 kohm.
XRIP COREP FILTP 0 rd_hr R=1500
XRIN COREN FILTN 0 rd_hr R=1500
XCFP FILTP 0 rd_c4p
XCFN FILTN 0 rd_c4p
...
"""
    ...
    instances.extend((
        "XVCMLO rd_case SUP=1.8 G=4 S0=1.8 S1=0 VREF=0.85",
        "XVCMHI rd_case SUP=1.8 G=4 S0=1.8 S1=0 VREF=0.95",
    ))
    ...
    for gain in GAINS:
        ...
        nodes = [
            ...
            f"v({p}.xdut.xota.ncm)", f"v({p}.xdut.xota.cms)",
            f"v({p}.xdut.xota.bn)",
    ...
    for name in ("xvcmlo", "xvcmhi"):
        nodes = [f"v({name}.vcm)", f"v({name}.filtp)", f"v({name}.filtn)"]
        op.extend(nodes)
        dc.extend(nodes)
        transient.extend(nodes)
...
.temp 27
.options reltol=1e-5 abstol=1e-14 vntol=1e-8 itl1=300 itl2=300 method=gear maxord=2 klu
VSW SW 0 PULSE(-.36 .36 10u 10n 10n 10u 20u)
VACQ ACQ 0 PULSE(0 1.8 10u 10n 10n {ACQ_WINDOW} 10u)
VACQB ACQB 0 PULSE(1.8 0 10u 10n 10n {ACQ_WINDOW} 10u)
* Explicit finite-slope PWL reset edges avoid a periodic-source breakpoint
* pathology seen at exactly 8.01 us.  Voltage levels, 10 ns edges, 1 us high
* time, and 10 us repetition are unchanged from the frozen stimulus.
VRST RST 0 PWL(0 0 8u 0 8.01u 1.8 9.01u 1.8 9.02u 0 18u 0 18.01u 1.8 19.01u 1.8 19.02u 0 28u 0 28.01u 1.8 29.01u 1.8 29.02u 0 38u 0 38.01u 1.8 39.01u 1.8 39.02u 0 40u 0)
VRSTB RSTB 0 PWL(0 1.8 8u 1.8 8.01u 0 9.01u 0 9.02u 1.8 18u 1.8 18.01u 0 19.01u 0 19.02u 1.8 28u 1.8 28.01u 0 29.01u 0 29.02u 1.8 38u 1.8 38.01u 0 39.01u 0 39.02u 1.8 40u 1.8)
...
def analyze(folder: Path, opcols: list[str], dccols: list[str], trcols: list[str]) -> dict:
    ...
    vcm_tracking = {}
    for name, target_vcm in (("xvcmlo", .85), ("xvcmhi", .95)):
        output_cm = (dc[:, ci_dc[f"v({name}.filtp)"]] + dc[:, ci_dc[f"v({name}.filtn)"]]) / 2
        error = output_cm - dc[:, ci_dc[f"v({name}.vcm)"]]
        vcm_tracking[str(target_vcm)] = {
            "external_vcm_v": target_vcm,
            "maximum_output_cm_tracking_error_v": float(np.max(np.abs(error))),
            "full_input_range_gate_pass": bool(np.max(np.abs(error)) <= .05),
        }
    vcm_tracking_pass = all(item["full_input_range_gate_pass"] for item in vcm_tracking.values())
    ...
    return {
        ...
        "external_vcm_reference_used": True,
        "external_vcm_endpoint_tracking": vcm_tracking,
        "external_vcm_endpoint_tracking_gate_pass": vcm_tracking_pass,
