...
def noise_deck(gain,state):
    lines=base(gain,state)+dut()+control()+['op','wrdata op.dat v(op) v(on) v(fp) v(fn) i(vdd) i(vcm)','ac dec 120 1 1g','let gain=mag(v(fp)-v(fn))','wrdata gain.dat gain','noise v(fp,fn) VINP dec 120 1 1g','wrdata noise_density.dat onoise_spectrum inoise_spectrum','setplot noise2','print all','write noise_integrated.raw all']+end()
    return '\n'.join(lines)+'\n',None
...
if __name__=='__main__':
    ...
    for g in a.gains:
        for state in a.states:run(a.kind,g,state,a.timeout)
