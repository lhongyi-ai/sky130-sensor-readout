...
def analyze_noise(folder,gain):
    ...
    return {'status':'STATIC_NOISE_DIAGNOSTIC_COMPLETE','output_rms_v':{str(hi):integral(hi) for hi in (5000,50000,1000000000)},'calibration_record':{k:v for k,v in gain_data.items() if 'calib' in k},'provisional_output_budget_v':113e-6,'below_legacy_provisional_screen_before_calibration':integral(1e9)<=113e-6,'formal_noise_gate_pass':None,'budget_scope':'Diagnostic comparison only: old FDDA filter and new phase-fixed resistor-input PGA are different test assemblies.','sampled_noise_signoff':False,'gain_reference_source_sha256':old['candidate_sha256']}
