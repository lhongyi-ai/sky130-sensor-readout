...
def convert(out, job, nodes, devices):
    ...
    with (out/'op_devices.csv').open('w') as f:
        ...
        for (name, kind), vals in zip(devices, b):
            vals=vals.copy()
            # ngspice BSIM OP uses type-normalized PMOS voltage/current;
            # the shared school analyzer expects signed D-S/G-S quantities.
            # Raw op.tsv stays untouched and records the original convention.
            if kind=='pfet_01v8': vals[[0,3,4,5]] *= -1
            w.writerow([name, *vals])
    put(out/'op_conversion.json',dict(raw='op.tsv',normalized='op_devices.csv',policy='PMOS BSIM type-normalized id/vgs/vds/vdsat multiplied by -1 for signed school-schema analysis; gm and gds unchanged.'))
