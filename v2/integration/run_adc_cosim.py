...
def validate_bridge(report, root, vdd):
    """A historical bridge PASS does not qualify changed RTL or wiring."""
    if (report.get("status") != "LIVE_RTL_SPICE_BRIDGE_PASS"
            or report.get("backend") != "verilator" or report.get("vdd_v") != vdd):
        raise ValueError("logic bridge qualification does not match this supply/backend")
    ...
    for name in ("integration/cosim_controller.v", "integration/qualify_cosim.py", "rtl/sar_controller.v"):
        ...
        if not path.is_file() or recorded.get(name) != hashlib.sha256(path.read_bytes()).hexdigest():
            raise ValueError(f"logic bridge source missing or changed: {name}; rerun qualification")
...
def main():
    ...
    parser.add_argument("--reference-cm-mode",choices=("tracking","fixed"),default="tracking")
    parser.add_argument("--source-r",type=float,choices=(0,350,1000),default=350)
    parser.add_argument("--input-cm-offset-mv",type=float,choices=(-50,0,50),default=0)
    ...
    parser.add_argument("--solver",choices=("sparse","klu"),default="sparse")
    parser.add_argument("--timeout-seconds",type=float,default=900)
    parser.add_argument("--frontend", type=Path, help="explicit frozen frontend PDK netlist snapshot")
    parser.add_argument("--frontend-riso",type=float,help="explicit physical isolation resistance per leg")
    ...
    if not -.4 <= args.input_v <= .4 or args.reference_r <= 0 or args.reference_c_nf <= 0 or args.duration_us < 12 or args.timeout_seconds <= 0:
    ...
    try:
        validate_bridge(json.loads(qualification.read_text()), ROOT, args.vdd)
    except ValueError as error:
        parser.error(str(error))
    out = HERE / "results" / ("adc_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ"))
    out.mkdir(parents=True)
    report_path = out / "summary.json"
    argument_report = {key: str(value) if isinstance(value,Path) else [str(p) for p in value] if key=="extra_include" else value for key,value in vars(args).items()}
    ...
    files = [HERE / "cosim_controller.v", HERE / "qualify_cosim.py", Path(__file__), ROOT / "rtl/sar_controller.v",
             args.adc_blocks.resolve(), args.adc_wrapper.resolve(), HERE / "sensor_phases.spice"]
    if args.adc_preamp:
        files.append(args.adc_preamp.resolve())
    if args.frontend:
        files.append(args.frontend.resolve())
    files.extend(path.resolve() for path in args.extra_include)
    source_hashes = {}
    for path in files:
    ...
    vcm = args.vdd/2
    refcm = vcm if args.reference_cm_mode=="tracking" else .9
    ...
    analog_frontend = ([f".include {args.frontend.name}",
        "XAFE sensorp sensorn inp inn vdd 0 vcm gain0 gain1 sky130_v2_switchable_sample_driver"+
            (f" RISO={args.frontend_riso}" if args.frontend_riso is not None else "")]
    ...
    lines = [f"SKY130 ADC real-transistor / unchanged-RTL mixed simulation; {args.phase_source} phases",
             ...
             f".include {args.adc_blocks.name}", f".include {args.adc_wrapper.name}", ".include sensor_phases.spice",
             *([f".include {args.adc_preamp.name}"] if args.adc_preamp else []),
             *[f".include {path.name}" for path in args.extra_include],
             f".temp {args.temperature}", f".options {args.solver} reltol=1e-4 abstol=1e-12 vntol=1e-7 method=gear",
             f"VDD vdd 0 {args.vdd}", f"VVCM vcm_ideal 0 {vcm}",
             f"VVRP rp_ideal 0 {refcm+.2}", f"VVRN rn_ideal 0 {refcm-.2}",
             ...
             f"VINP vinp 0 {vcm+args.input_cm_offset_mv/1000+args.input_v/2/sensor_gain}", f"VINN vinn 0 {vcm+args.input_cm_offset_mv/1000-args.input_v/2/sensor_gain}",
             *source_connections, *analog_frontend,
             ...
             ".save v(clk) v(top_sample) v(acq) v(conv) v(evaluate) v(valid) v(xadc.tp) v(xadc.tn) v(decision) v(decision_b) v(rp) v(rn) v(vcm) i(vdd) i(vportp) i(vportn) i(vportcm)",
    ...
    if waveform.exists():
        ...
        if len(edges)>=2:
            begin,end = time[edges[0]+1],time[edges[-1]+1]
            use = (time>=begin)&(time<=end)
            # Complete conversion periods, excluding the first conversion.
            # Device-side reference energy, not generator static power.
            power = -args.vdd*values[:,14]+values[:,11]*values[:,15]+values[:,12]*values[:,16]+values[:,13]*values[:,17]
            report["analog_and_reference_port_power_w"] = float(np.trapezoid(power[use],time[use])/(end-begin))
            report["power_window_s"] = [float(begin),float(end)]
            report["power_scope"] = "Whole conversion periods between valid edges; schematic analog/phase/reference port power, excluding ideal digital bridges"
            ...
            report["power_not_reported_reason"] = "At least two complete valid edges are needed for a whole-conversion-period power window."
        count = len(edges)
    else:
        count = 0
    ...
    report["status"] = ("SHORT_LIVE_RTL_TRANSISTOR_CONVERSION_PASS" if all(checks.values()) else
                        "SIMULATION_TIMEOUT_INCOMPLETE" if rc is None else "FAIL")
