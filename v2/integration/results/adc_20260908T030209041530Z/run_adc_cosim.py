...
def main():
    ...
    files = [HERE / "cosim_controller.v", HERE / "qualify_cosim.py", Path(__file__), ROOT / "rtl/sar_controller.v",
             ROOT / "analog/adc/adc_blocks.spice", ROOT / "analog/adc/adc_analog12.spice", HERE / "sensor_phases.spice"]
    if args.frontend:
    ...
    lines = ["SKY130 ADC real-transistor / unchanged-RTL mixed simulation; ideal phase fixture",
             ...
             ".include adc_blocks.spice", ".include adc_analog12.spice", ".include sensor_phases.spice",
             ...
             "RINP vinp sensorp 350", "RINN vinn sensorn 350", *analog_frontend,
             "Vclk clk 0 PULSE(0 1.8 1u 1n 1n 311.5n 625n)",
             "Vrst rst 0 PWL(0 0 200n 0 201n 1.8)",
             "Vstart start 0 PWL(0 0 800n 0 801n 1.8)",
             f"Vg1 gain_sel1 0 {1.8*(gain_code//2)}", f"Vg0 gain_sel0 0 {1.8*(gain_code%2)}", *controller_elements(binary, "verilator"), *phases,
